...
<title><fmt:message key="title_page" />
</title>

<link rel="stylesheet" type="text/css" href="<%= request.getContextPath()%>/${it.externalRessourceCtx}/css/common-tablet.css" />
<link rel="stylesheet" type="text/css" href="${it.externalCssUrl}scenario2-tablet.css" />

<!-- jQuery -->
<script src="<%=request.getContextPath()%>/../browserscript/js/jquery-1.7.1.min.js" type="text/javascript"></script>

<script type="text/javascript"
    src="<%=request.getContextPath()%>/../browserscript/js/jquery.simplemodal.js"></script>
<c:if test="${!empty it.authentication && it.authentication}">
    <!-- pinPad -->
    <script type="text/javascript"
        src="<%=request.getContextPath()%>/../browserscript/js/dictao.js"></script>
    <script type="text/javascript"
        src="<%=request.getContextPath()%>/../browserscript/js/dictao.kit.pinpad.js"></script>
    <script type="text/javascript">    
...
<link rel="stylesheet" type="text/css"
    href="<%=request.getContextPath()%>/${it.externalRessourceCtx}/css/docviewer.css" />
...
<script type="text/javascript">      
    ...
    function resize(){
        $("._content").height($("#viewCast").height() - $("._header").height() -  $("._footer").height());
    }
        ...
        <div class="_container">
            <div class="_header">
                ...
                <div class="_document_progress"> 
                    ...
                    ${it.current}/${it.count} : ${it.current_document.label}
                </div>		
            </div>
            ...
            <div class="_footer">
                ...
                <div id="footer" align="center">
                    <table>
                        <tr>
                            <td>
                                <form id="" method="post" action="consult-and-agreement?current=${it.current - 1}">
                                    <button type="submit" onclick="openLaoder();"
                                        class="_va_btn"
                                        <c:if test="${it.current == 1}"> disabled </c:if> ><fmt:message key="button_previous" /></button>
                                </form></td>

                            <td class="_spacer"></td>
                            <td><button class="_an_btn" type="button"
                                onclick="location.href='cancel'; openLaoder();"
                                ><fmt:message key="button_cancel" /></button>
                            </td>
                            <td class="_spacer"></td>
                            <td><button id="validateButton" class="_va_btn" type="button"
                                <c:if test="${ empty it.allDocumentsSeen || !it.allDocumentsSeen}"> disabled </c:if>
                                onclick="openModal(${it.authentication});"><fmt:message key="button_valid" /></button>
                            </td>
                            <td class="_spacer"></td>
                            <td>
                                <form id="" method="post"
                                    action="consult-and-agreement?current=${it.current + 1}">
                                    <button type="submit" onclick="openLaoder();"
                                        class="_va_btn"
                                        <c:if test="${it.current == it.count}"> 
                                        disabled </c:if>><fmt:message key="button_next" /></button>
                                </form>
                            </td>
                        </tr>
                    </table>
            ...
            <form method="post" id="signature" action="signature">
                ...
                <c:if test="${!empty it.term_and_conditions}">
                    <div class="_signature_popup_term_and_conditions">
                        <input id="consentCheckBox" type="checkbox" name="consentCheckBox" class="_signature_popup_checkbox"
                ...
                <input type="hidden" name="currentPage" id="currentPage" value="consult-and-agreement?current=${it.current}" />
